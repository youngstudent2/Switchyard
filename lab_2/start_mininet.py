...
class PySwitchTopo(Topo):
    ...
    def __init__(self, args):
        ...
        for node in ['server1','server2','server3','server4','server5','client']:
...
def setup_addressing(net):
    ...
    reset_macs(net, 'server4', '40:00:00:00:00:{:02x}')
    reset_macs(net, 'server5', '50:00:00:00:00:{:02x}')
    reset_macs(net, 'client', '60:00:00:00:00:{:02x}')
    reset_macs(net, 'switch', '70:00:00:00:00:{:02x}')
    set_ip(net, 'server1','switch','192.168.100.1/24')
    set_ip(net, 'server2','switch','192.168.100.2/24')
    set_ip(net, 'server3','switch','192.168.100.3/24')
    set_ip(net, 'server4','switch','192.168.100.4/24')
    set_ip(net, 'server5','switch','192.168.100.5/24')
    set_ip(net, 'client','switch','192.168.100.6/24')
